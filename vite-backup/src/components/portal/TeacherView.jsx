...
export default function TeacherView({ activeTab, setActiveTab }) {
  const [attendance, setAttendance] = useState(() => {
    const saved = localStorage.getItem('teacher_attendance');
    return saved ? JSON.parse(saved) : STUDENTS_MOCK.reduce((acc, student) => {
  ...
  const handleMarkGrade = (studentId, course, mark) => {
  ...
  const handleCreateQuiz = (e) => {
    ...
    setQuizTitle('');
    setQuizQuestion('');
    setTimeout(() => setQuizMessage(''), 3000);
  };
        ...
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <form onSubmit={handleUploadLesson} className="bg-gray-900 border border-gray-800 rounded-2xl p-6 space-y-4">
              ...
              <div>
                <label className="block text-[10px] text-gray-400 uppercase font-bold mb-1">Resource Link / File Name</label>
                <input
                  type="text"
