...
function PDFViewerModal({ course, initialChapterIndex, onClose }: PDFViewerModalProps) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-80 p-2 md:p-4">
      <div className="relative bg-gray-900 rounded-xl shadow-2xl w-full h-full max-w-7xl max-h-[95vh] flex flex-col">
        ...
        <div className="flex items-center justify-between px-4 md:px-6 py-3 border-b border-gray-700 flex-shrink-0">
          ...
          <button
            onClick={onClose}
            className="text-gray-400 hover:text-white transition-colors p-2 rounded-lg hover:bg-gray-700 cursor-pointer"
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className="w-52 md:w-64 bg-gray-800 flex-shrink-0 overflow-y-auto border-r border-gray-700">
            <p className="text-gray-400 text-xs font-semibold uppercase tracking-wider px-4 py-3 border-b border-gray-700">
              Chapters
            </p>
            <ul>
              {course.chapters.map((ch, idx) => (
                <li key={ch.id}>
                  <button
                    ...
                    className={`w-full text-left px-4 py-3 text-sm border-b border-gray-700 transition-colors flex items-start gap-2 cursor-pointer ${
                      idx === activeChapter
          ...
          <div className="flex-1 flex flex-col bg-gray-950 overflow-hidden">
            ...
            <div className="flex items-center justify-between px-4 py-2 bg-gray-800 border-b border-gray-700 flex-shrink-0">
              <button
                ...
                className="flex items-center gap-2 text-sm text-gray-300 hover:text-white disabled:opacity-30 disabled:cursor-not-allowed transition-colors cursor-pointer"
              >
...
interface CourseCardProps {
  course: Course;
  onOpen: (course: Course, index: number) => void;
}
...
function CourseCard({ course, onOpen }: CourseCardProps) {
  ...
  return (
    <div className="bg-white rounded-xl shadow-md hover:shadow-xl transition-all duration-300 flex flex-col overflow-hidden border border-gray-100 group">
      {/* Card Top Bar */}
      <div
      ...
      <div className="p-5 flex flex-col flex-1">
        <div className="flex items-start justify-between mb-3">
          <span
            className={`text-xs font-semibold px-2.5 py-1 rounded-full capitalize ${
              categoryColor[course.category] || 'bg-gray-100 text-gray-800'
            }`}
          >
            {course.category}
          </span>
        ...
        <h3 className="font-bold text-gray-900 text-base mb-1 leading-snug group-hover:text-custom-orange transition-colors">
          {course.title}
        </h3>
          ...
          {course.chapters.slice(0, 3).map((ch, idx) => (
            <li key={ch.id}>
              <button
                ...
                className="flex items-center gap-2 w-full text-left text-sm text-blue-600 hover:text-blue-800 hover:underline transition-colors cursor-pointer"
              >
        ...
        <button
          ...
          className="mt-auto w-full bg-gray-900 hover:bg-custom-orange text-white text-sm font-semibold py-2.5 px-4 rounded-lg transition-colors duration-300 flex items-center justify-center gap-2 cursor-pointer"
        >
...
export default function Courses() {
  ...
  const [viewer, setViewer] = useState<{ course: Course; chapterIndex: number } | null>(null);
  ...
  return (
    <section id="courses" className="py-16 px-4 md:px-10 bg-gray-50 text-gray-900">
      {/* Section Header */}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 max-w-screen-xl mx-auto">
          {filtered.map((course) => (
            <CourseCard
              key={course.id}
              course={course}
              onOpen={(c, idx) => setViewer({ course: c, chapterIndex: idx })}
            />
          ))}
        </div>
      )}
